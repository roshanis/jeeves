...
describe("evidence-led review", () => {
  it("keeps shared sample review commands read-only even with an assigned live reviewer", async () => {
    ...
    const row: ReviewQueueRow = { slug: "sample", title: "Sample", tier: "high", isSeeded: true, review: { cycleId: "cycle", draftToken: "a".repeat(64), domain: "privacy-hipaa", status: "drafted", reviewer: null, createdAt: "2026-09-19T12:00:00Z", signedAt: null, draftMd: "Sample draft", citations: [] } };
  ...
  it("binds human edits to the original reviewed draft even when refreshed rows contain a replacement", async () => {
    ...
    state.requirements[0].status = "accepted";
    mocks.request.mockResolvedValue(state);
    const row: ReviewQueueRow = { slug: "case", title: "Case", tier: "high", review: { cycleId: "cycle", draftToken: "a".repeat(64), domain: "privacy-hipaa", status: "drafted", reviewer: null, createdAt: "2026-09-19T12:00:00Z", signedAt: null, draftMd: "Original agent draft", citations: [] } };
    function Harness() {
      const [replaced, setReplaced] = useState(false);
      return <><button onClick={() => setReplaced(true)}>Refresh replaced draft</button><ReviewWorkbench rows={[{ ...row, review: { ...row.review, draftToken: (replaced ? "b" : "a").repeat(64), draftMd: replaced ? "Concurrent replacement" : "Original agent draft" } }]} selection={{ slug: "case", domain: "privacy-hipaa" }} /></>;
    }
    ...
    await waitFor(() => expect(mocks.mutate).toHaveBeenCalledWith("reviewer-token", "cycle", "privacy-hipaa", {
      kind: "sign", editedDraftMd: undefined, expectedDraftToken: "b".repeat(64),
    }));
  });

  it("opens a supplied case/domain selection and preserves edits across URL-driven selection changes", async () => {
    const rows: ReviewQueueRow[] = ["one", "two"].map((slug) => ({ slug, title: `Case ${slug}`, tier: "high", review: { cycleId: "cycle", domain: "privacy-hipaa", status: "drafted", reviewer: null, createdAt: "2026-09-19T12:00:00Z", signedAt: null, draftMd: "Agent draft", citations: [] } }));
  ...
  it("uses the authenticated evidence cycle after a reload and enables sign only when the packet is accepted", async () => {
    ...
    state.requirements[0].status = "accepted";
    mocks.request.mockResolvedValue(state);
    const row: ReviewQueueRow = { slug: "unregistered", title: "Reloaded case", tier: "high", review: { draftToken: "a".repeat(64), domain: "privacy-hipaa", status: "drafted", reviewer: null, createdAt: "2026-09-19T12:00:00Z", signedAt: null, draftMd: "Agent draft", citations: [] } };
    function Harness() {
