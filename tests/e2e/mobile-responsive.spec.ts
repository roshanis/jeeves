...
for (const [label, viewport] of [
  ...
  test.describe(`${label} (${viewport.width}x${viewport.height})`, () => {
    test.use({ ...devices["Desktop Chrome"], viewport, isMobile: true, hasTouch: true });
        ...
        const undersized = await page.evaluate(() => {
          ...
          for (const el of Array.from(document.querySelectorAll(SELECTOR))) {
            ...
            for (const [px, py] of probes) {
              // Points outside the viewport cannot be probed; do not count
              // them as failures.
...
test.describe("iPhone landscape dialogs", () => {
  ...
  // scroll to it.
  test.use({ ...devices["Desktop Chrome"], viewport: { width: 852, height: 393 }, isMobile: true, hasTouch: true });
