...
### Nonblocking domain-reviewer abstention amendment (human request 2026-09-21)

A domain reviewer may abstain with a required reason. A valid, exact-revision audit receipt removes that reviewer from the remaining participation requirement for the cycle. Full approval requires every other required domain to be signed; conditional approval requires every other required domain to be drafted or signed. Missing, pending, returned, or unverified-abstention rows remain blocking. If all required reviewers abstain, the initiative still waits for an explicit decision by the named human approver; agents and Admin never approve.

Abstention is not a signature or an approval. Progress displays distinguish signed and abstained counts, and the final decision audit preserves the abstaining reviewer, reason, and receipt. Abstained reviewers remain visible without being counted as overdue reviewer work. They can explicitly resume before the cycle closes, restoring their prior participation requirement. A final decision closes the cycle and prevents resumption. Existing role, workspace, evidence, control, exception, and deployment requirements remain in force. This supersedes the earlier all-required-signatures/drafts rule only for valid recorded abstentions.
