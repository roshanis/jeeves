...
3. **Drafting surface** (appears on selecting a queue row; can be a side panel or a dedicated sub-view within the same page — recommend a two-pane layout so the queue stays visible):
   - **Left pane — agent draft + policy citations (read-only reference):** the agent-generated draft assessment text, plus a citations list linking to the specific policy sections (`docs/policies/*.md` anchors, e.g., MP-R-2.4 for #9's bias-testing gap) that the draft cites. All 8 domains are drafted live via the bounded-concurrency fan-out (per plan §1), so this pane always shows a live-drafted assessment for every domain — there is no seeded-vs-live distinction left to hide, and no dev-only badge is needed.
