/**
 * POST /api/exceptions/[id]/renew — open a new requested exception that
 * supersedes an approved one (to be approved via .../decide). Any
 * authenticated session may renew; the old exception must be approved.
 *   Body: { reason: string }
 */
import { z } from "zod";
import { getDb } from "@/lib/db/client";
import { runMutationGuard } from "@/lib/services/route-guard";
import { renewException } from "@/lib/services/exception-service";
